...
function playMyAudio(cat, s) {
    ...
    if (cat == 'ambient') {
        ...
        if (sound != null) {
            ...
            ambient_audio.addEventListener(
                'ended',
                function () {
                    this.currentTime = 0;
                    this.play();
        ...
        if (topping_audio != null) {
            topping_audio.pause();
            topping_audio = null;
        }
        if (sound != null) {
            topping_audio = new Audio(sound.mp3);
            topping_audio.addEventListener(
                'ended',
                function () {
                    this.currentTime = 0;
                    this.play();
...
function playASMR() {
    ...
    const updateVolume = function (x, vol) {
        ...
        topping_audio.volume = percentage / 100;

        //change sound icon based on volume
        if (topping_audio.volume == 0) {
            $('.sound').removeClass('sound2').addClass('muted');
        } else if (topping_audio.volume > 0.5) {
            $('.sound').removeClass('muted').addClass('sound2');
        } else {
            $('.sound').removeClass('muted').removeClass('sound2');
        }
